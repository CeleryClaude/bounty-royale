...
const BND = 52, EVOLVE_AT = 16, EVOLVE2_AT = 40;
const CLASSES = {
  juggernaut:{n:'Juggernaut',hp:1.7,dmg:1.3,spd:0.86,grow:1.08,hide:1.3,steal:1,reach:1,col:'#D85A30',cd:7000,ab:'Shockwave'},
  scout:{n:'Scout',hp:0.8,dmg:0.95,spd:1.4,grow:0.88,hide:2.25,steal:1,reach:1,col:'#1D9E75',cd:3200,ab:'Dash'},
  rogue:{n:'Rogue',hp:1.05,dmg:1.1,spd:1.1,grow:1,hide:1.7,steal:1.6,reach:1,col:'#7F77DD',cd:7500,ab:'Vanish'},
  skirmisher:{n:'Skirmisher',hp:0.9,dmg:1.0,spd:1.25,grow:0.95,hide:1.9,steal:1.2,reach:1.3,col:'#5DCAA5',cd:2600,ab:'Dart'},
  charger:{n:'Charger',hp:1.4,dmg:1.2,spd:1.15,grow:1.05,hide:1.4,steal:1,reach:1.05,col:'#EF9F27',cd:6000,ab:'Charge'},
  reaper:{n:'Reaper',hp:1.25,dmg:1.45,spd:0.95,grow:1.05,hide:1.5,steal:1.7,reach:1.1,col:'#993556',cd:8000,ab:'Execute'},
  lancer:{n:'Lancer',hp:1.1,dmg:1.2,spd:1.3,grow:1.0,hide:1.7,steal:1.2,reach:1.35,col:'#36c5d6',cd:2400,ab:'Volley'},
  warlord:{n:'Warlord',hp:1.8,dmg:1.5,spd:0.9,grow:1.1,hide:1.2,steal:1.2,reach:1.1,col:'#b5532a',cd:6500,ab:'Quake'},
  phantom:{n:'Phantom',hp:1.2,dmg:1.6,spd:1.1,grow:1.0,hide:1.8,steal:1.9,reach:1.15,col:'#6a3aa0',cd:7000,ab:'Deathmark'},
  summoner:{n:'Summoner',hp:1.0,dmg:0.55,spd:0.95,grow:1.0,hide:1.6,steal:1.1,reach:1.0,col:'#48b59a',cd:7000,ab:'Swarm',minions:2},
  hivemind:{n:'Hivemind',hp:1.15,dmg:0.62,spd:1.0,grow:1.0,hide:1.6,steal:1.2,reach:1.0,col:'#2f9fbf',cd:6000,ab:'Swarm',minions:3},
  broodlord:{n:'Broodlord',hp:1.4,dmg:0.72,spd:0.95,grow:1.05,hide:1.4,steal:1.35,reach:1.4,col:'#1f7fb0',cd:6000,ab:'Swarm',minions:5},
  trapper:{n:'Trapper',hp:1.1,dmg:0.7,spd:0.92,grow:1.0,hide:1.5,steal:1.1,reach:1.0,col:'#d8a23a',cd:5000,ab:'Snare',traps:3},
  warden:{n:'Warden',hp:1.25,dmg:0.8,spd:0.92,grow:1.0,hide:1.5,steal:1.2,reach:1.1,col:'#e07a2a',cd:4500,ab:'Snare',traps:4},
  citadel:{n:'Citadel',hp:1.6,dmg:0.9,spd:0.85,grow:1.05,hide:1.3,steal:1.3,reach:1.2,col:'#c4561f',cd:4000,ab:'Snare',traps:6},
  marksman:{n:'Marksman',hp:0.85,dmg:0.9,spd:1.05,grow:0.95,hide:1.5,steal:1.2,reach:1.0,col:'#6fbf73',cd:6000,ab:'Volley',ranged:true,fireRate:0.75},
  sharpshooter:{n:'Sharpshooter',hp:0.9,dmg:1.1,spd:1.05,grow:0.95,hide:1.5,steal:1.3,reach:1.0,col:'#4fae5a',cd:5500,ab:'Volley',ranged:true,fireRate:0.62},
  deadeye:{n:'Deadeye',hp:1.0,dmg:1.5,spd:1.05,grow:0.95,hide:1.6,steal:1.5,reach:1.0,col:'#2f9e57',cd:5000,ab:'Volley',ranged:true,fireRate:0.5}
};
const UPGRADES = { rogue:['skirmisher','reaper'], scout:['skirmisher','charger'], juggernaut:['charger','reaper'], summoner:['hivemind'], trapper:['warden'], marksman:['sharpshooter'] };
const UPGRADES2 = { skirmisher:['lancer','phantom'], charger:['lancer','warlord'], reaper:['warlord','phantom'], hivemind:['broodlord'], warden:['citadel'], sharpshooter:['deadeye'] };
const baseKeys = ['juggernaut','scout','rogue','summoner','trapper','marksman'];
...
const aHp=e=>1+((e.attr&&e.attr.hp)||0)*0.10;
const aDmg=e=>1+((e.attr&&e.attr.dmg)||0)*0.08;
const aSpd=e=>1+Math.min(0.5,((e.attr&&e.attr.spd)||0)*0.05);
const aReach=e=>((e.attr&&e.attr.reach)||0)*0.06;
const radOf = e => scaleOf(e.points, e.cls)*0.95;
const reachRad = e => radOf(e)*((e.cls.reach||1)+aReach(e));
const maxHp = e => 100*e.cls.hp*(1 + (scaleOf(e.points,e.cls)-1)*0.8)*aHp(e);
...
function spawnEnt(R,o){
  const id = R.nextId++;
  const baseKey = o.clsKey || baseKeys[Math.floor(Math.random()*baseKeys.length)];
  const cls = CLASSES[baseKey];
  const e = { id, room:R, bot:!!o.bot, ws:o.ws||null, prof:o.prof||null, name:(o.name||('P'+id)).slice(0,12),
    baseKey, clsKey:baseKey, cls, tier:0, color:o.color||cls.col, cosColor:null, hatVal:0, finishVal:'matte', trailVal:0, attr:{hp:0,dmg:0,spd:0,regen:0,life:0,reach:0}, pts:0, matchXp:0, matchLvl:1, minions:[], swarmAt:0, fireCd:0,
...
function evolve(e,to){ let ok=false; if(e.tier===0&&e.points>=EVOLVE_AT&&UPGRADES[e.baseKey]&&UPGRADES[e.baseKey].includes(to))ok=true; else if(e.tier===1&&e.points>=EVOLVE2_AT&&UPGRADES2[e.clsKey]&&UPGRADES2[e.clsKey].includes(to))ok=true; if(!ok)return; const nt=e.tier+1; e.clsKey=to; e.cls=CLASSES[to]; e.tier=nt; e.abilAt=nowMs; if(!e.cosColor)e.color=e.cls.col; if(e.hp>maxHp(e))e.hp=maxHp(e); e.room.events.push({k:'evo',who:e.name,cls:e.cls.n}); }
function useAbility(e){
  if(nowMs<e.abilAt)return; e.abilAt=nowMs+e.cls.cd; const k=e.clsKey, R=e.room;
  if(k==='summoner'||k==='hivemind'||k==='broodlord'){ e.swarmAt=nowMs+3200; }
  else if(k==='trapper'||k==='warden'||k==='citadel'){ dropTrap(e); }
  else if(k==='marksman'||k==='sharpshooter'||k==='deadeye'){ for(const off of [-0.3,-0.15,0,0.15,0.3]){ const aa=e.a+off,sp=42; R.projs.push({x:e.x+Math.sin(aa)*1.5,z:e.z+Math.cos(aa)*1.5,vx:Math.sin(aa)*sp,vz:Math.cos(aa)*sp,own:e.id,life:0.6,dmg:10*e.cls.dmg*aDmg(e)+4}); } }
  else if(k==='scout'){ e.dashUntil=nowMs+320; }
  else if(k==='rogue'){ e.vanishUntil=nowMs+1900; }
  else if(k==='charger'){ e.dashUntil=nowMs+550; e.chargeUntil=nowMs+550; }
  else if(k==='lancer'){ for(const off of [-0.22,0,0.22]){ const aa=e.a+off,sp=38; R.projs.push({x:e.x+Math.sin(aa)*1.5,z:e.z+Math.cos(aa)*1.5,vx:Math.sin(aa)*sp,vz:Math.cos(aa)*sp,own:e.id,life:0.55,dmg:15*e.cls.dmg*aDmg(e)+5}); } }
  else if(k==='skirmisher'){ const sp=38; R.projs.push({x:e.x+Math.sin(e.a)*1.5,z:e.z+Math.cos(e.a)*1.5,vx:Math.sin(e.a)*sp,vz:Math.cos(e.a)*sp,own:e.id,life:0.55,dmg:16*e.cls.dmg*aDmg(e)+6}); }
  else if(k==='juggernaut'||k==='warlord'){ const Rr=(k==='warlord'?10:8)+scaleOf(e.points,e.cls)*2; const dm=(k==='warlord'?24:18)*e.cls.dmg*aDmg(e); for(const o of R.ents.values()){ if(o===e)continue; const dx=o.x-e.x,dz=o.z-e.z,d=Math.hypot(dx,dz); if(d<Rr&&d>0){ const push=(Rr-d)/Rr*6; o.x+=dx/d*push;o.z+=dz/d*push;clampPos(o); if(o.vanishUntil<nowMs&&o.safe<nowMs){o.hp-=dm;o.lastHit=nowMs;if(o.hp<=0)kill(e,o);} } } }
  else if(k==='reaper'||k==='phantom'){ let best=null,bd=1e9; const Rr=reachRad(e)+(k==='phantom'?4.5:3.5); for(const o of R.ents.values()){ if(o===e||o.safe>nowMs||o.vanishUntil>nowMs)continue; const d=dist(e,o); if(d<Rr&&d<bd){bd=d;best=o;} } if(best){ const dmg=(k==='phantom'?40:32)*e.cls.dmg*aDmg(e); best.hp-=dmg;best.lastHit=nowMs; const dx=best.x-e.x,dz=best.z-e.z,dd=Math.hypot(dx,dz)||1; best.x+=dx/dd*3;best.z+=dz/dd*3;clampPos(best); e.hp=Math.min(maxHp(e),e.hp+dmg*0.4); if(k==='phantom')e.vanishUntil=nowMs+900; if(best.hp<=0)kill(e,best); } }
  R.events.push({k:'ab',who:e.name,ab:e.cls.ab,x:Math.round(e.x),z:Math.round(e.z)});
}
function botAI(b){
  const R=b.room;
  if(b.tier===0&&b.points>=EVOLVE_AT&&nowMs>b.evoWait){ const o=UPGRADES[b.baseKey]; if(o)evolve(b,o[Math.floor(Math.random()*o.length)]); } else if(b.tier===1&&b.points>=EVOLVE2_AT&&nowMs>b.evoWait){ const o=UPGRADES2[b.clsKey]; if(o)evolve(b,o[Math.floor(Math.random()*o.length)]); }
  ...
  if(nowMs>=b.abilAt){ const k=b.clsKey; if((k==='summoner'||k==='hivemind'||k==='broodlord'||k==='trapper'||k==='warden'||k==='citadel')&&(th||(prey&&pd<18)))useAbility(b); else if((k==='marksman'||k==='sharpshooter'||k==='deadeye'||k==='lancer'||k==='skirmisher')&&prey&&pd<22)useAbility(b); else if((k==='scout'||k==='charger')&&(th||(prey&&pd<14)))useAbility(b); else if((k==='rogue'||k==='phantom')&&(b.hp<maxHp(b)*0.4||(prey&&pd<6)))useAbility(b); else if(k==='juggernaut'||k==='reaper'||k==='warlord'){for(const o of R.ents.values()){if(o!==b&&dist(o,b)<6.5){useAbility(b);break;}}} }
}
function step(e,dt){ const dx=e.tx-e.x,dz=e.tz-e.z,d=Math.hypot(dx,dz); let base=(e.bot?12:15)*e.cls.spd*aSpd(e); if(e.dashUntil>nowMs)base*=2.7; const v=base*dt; if(d>0.5){e.x+=dx/d*v;e.z+=dz/d*v;e.a=Math.atan2(dx,dz);e.moving=1;}else e.moving=0; clampPos(e); }
...
function ensureMinions(e){ const want=(e.cls&&e.cls.minions)||0; if(!e.minions)e.minions=[]; while(e.minions.length<want)e.minions.push({ang:rand(0,6.28),cd:rand(0,0.5),x:e.x,z:e.z}); while(e.minions.length>want)e.minions.pop(); }
function updateMinions(R,dt){ const all=[...R.ents.values()]; for(const e of all){ ensureMinions(e); if(!e.minions.length)continue; const swarm=e.swarmAt>nowMs; const orbR=radOf(e)+1.7+(swarm?0.6:0); const osp=swarm?3.6:2.0; const reach=(0.62+radOf(e)*0.12)*(swarm?1.25:1)+0.55; const dps=(swarm?13:9)*(e.cls.dmg+0.35); const n=e.minions.length; const oh=e.vanishUntil>nowMs; for(let idx=0;idx<n;idx++){ const m=e.minions[idx]; m.ang+=dt*osp; const a=m.ang+idx/n*6.283; m.x=e.x+Math.cos(a)*orbR; m.z=e.z+Math.sin(a)*orbR; m.cd-=dt; if(oh)continue; for(const o of all){ if(o===e)continue; if(o.vanishUntil>nowMs||o.safe>nowMs||o.hidden)continue; const dx=o.x-m.x,dz=o.z-m.z,rr=radOf(o)+reach; if(dx*dx+dz*dz<rr*rr){ if(m.cd<=0){ const dmg=dps*0.5; o.hp-=dmg;o.lastHit=nowMs; if(e.attr&&e.attr.life>0)e.hp=Math.min(maxHp(e),e.hp+dmg*e.attr.life*0.16); if(o.hp<=0)kill(e,o); m.cd=0.5; } break; } } } } }
function dropTrap(e){ const cap=(e.cls&&e.cls.traps)||0; if(!cap)return; const R=e.room; const r=3.0+e.tier*0.7; R.traps.push({x:e.x,z:e.z,r,born:nowMs,life:9000,own:e.id}); let mine=R.traps.filter(t=>t.own===e.id); while(mine.length>cap){ const old=mine.shift(); const ix=R.traps.indexOf(old); if(ix>=0)R.traps.splice(ix,1); } }
function updateTraps(R,dt){ const all=[...R.ents.values()]; for(let i=R.traps.length-1;i>=0;i--){ const tp=R.traps[i]; if(nowMs-tp.born>tp.life){ R.traps.splice(i,1); continue; } const owner=R.ents.get(tp.own); const odmg=owner?owner.cls.dmg:0.7; for(const o of all){ if(o.id===tp.own)continue; if(o.vanishUntil>nowMs||o.safe>nowMs||o.hidden)continue; const dx=o.x-tp.x,dz=o.z-tp.z,d=Math.hypot(dx,dz); if(d<tp.r){ o.hp-=11*odmg*dt; o.lastHit=nowMs; if(d>0.3){o.x-=dx/d*dt*2.4;o.z-=dz/d*dt*2.4;clampPos(o);} if(o.hp<=0&&owner)kill(owner,o); } } } }
function autoFire(R,dt){ const all=[...R.ents.values()]; for(const e of all){ if(!e.cls.ranged||e.safe>nowMs||e.hidden)continue; e.fireCd=(e.fireCd||0)-dt; if(e.fireCd>0)continue; let best=null,bd=1e9; const Rr=18+e.tier*2; for(const o of all){ if(o===e||o.safe>nowMs||o.vanishUntil>nowMs||o.hidden)continue; const d=Math.hypot(o.x-e.x,o.z-e.z); if(d<Rr&&d<bd){bd=d;best=o;} } if(!best){e.fireCd=0.2;continue;} const aa=Math.atan2(best.x-e.x,best.z-e.z); e.a=aa; const sp=40; R.projs.push({x:e.x+Math.sin(aa)*1.4,z:e.z+Math.cos(aa)*1.4,vx:Math.sin(aa)*sp,vz:Math.cos(aa)*sp,own:e.id,life:0.62,dmg:7*e.cls.dmg*aDmg(e)+3}); e.fireCd=e.cls.fireRate||0.7; } }
function levelUp(e){ if(e.bot){ const ks=['hp','dmg','spd','regen','life','reach']; const kk=ks[Math.floor(Math.random()*ks.length)]; if(e.attr[kk]<7)e.attr[kk]++; } else { e.pts=(e.pts||0)+1; } }
function gainMatchXp(e,n){ e.matchXp=(e.matchXp||0)+n; let need=4+e.matchLvl*3; while(e.matchXp>=need){ e.matchXp-=need; e.matchLvl++; levelUp(e); need=4+e.matchLvl*3; } }
function simRoom(R,dt){
  ...
  for(const e of R.ents.values()){ const fr=radOf(e); for(let i=R.orbs.length-1;i>=0;i--){const o=R.orbs[i];if(Math.hypot(e.x-o.x,e.z-o.z)<fr+0.5){e.points+=1;gainMatchXp(e,1);R.orbs.splice(i,1);}} }
...
function snapshot(R){
  ...
  for(const e of R.ents.values()){ const cr=Math.max(0,Math.min(1,(e.abilAt-nowMs)/e.cls.cd));
    es.push({ i:e.id,x:Math.round(e.x*100)/100,z:Math.round(e.z*100)/100,a:Math.round(e.a*100)/100, s:Math.round(scaleOf(e.points,e.cls)*1000)/1000, p:e.points, hp:Math.round(e.hp), mh:Math.round(maxHp(e)), hd:e.hidden?1:0, sf:e.safe>nowMs?1:0, vn:e.vanishUntil>nowMs?1:0, ds:e.dashUntil>nowMs?1:0, tr:e.tier, ce:((e.tier===0&&e.points>=EVOLVE_AT)||(e.tier===1&&e.points>=EVOLVE2_AT))?1:0, cr:Math.round(cr*100)/100, n:e.name, co:e.cosColor||e.color, ht:e.hatVal||'none', fn:e.finishVal||'matte', tl:0, pts:e.pts, att:[e.attr.hp,e.attr.dmg,e.attr.spd,e.attr.regen,e.attr.life,e.attr.reach], lv:e.matchLvl, c:e.clsKey }); }
  return { t:'s', e:es, o:R.orbs.map(o=>[Math.round(o.x*10)/10,Math.round(o.z*10)/10]), pr:R.projs.map(p=>[Math.round(p.x*10)/10,Math.round(p.z*10)/10]), mn:[].concat(...[...R.ents.values()].map(en=>(en.minions||[]).map(m=>[Math.round(m.x*10)/10,Math.round(m.z*10)/10,en.cosColor||en.color]))), tp:R.traps.map(t=>[Math.round(t.x*10)/10,Math.round(t.z*10)/10,Math.round(t.r*10)/10,(R.ents.get(t.own)||{}).cosColor||(R.ents.get(t.own)||{}).color||'#d8a23a']), ev:R.events };
}
...
wss.on('connection',(ws)=>{
  ...
  ws.on('message',(buf)=>{ let m; try{m=JSON.parse(buf);}catch(_){return;}
    if(m.t==='join'&&!ent){
      ...
      ws.send(JSON.stringify({t:'w',id:ent.id,bnd:BND,igloos,upgrades:UPGRADES,upgrades2:UPGRADES2,classes:CLASSES,evolveAt:EVOLVE_AT,evolve2At:EVOLVE2_AT,token,profile:pubProfile(prof),cosmetics:COSMETICS,bundleUsd:BUNDLE_USD,top:topLeaders(),room:R.id}));
    ...
    else if(m.t==='attr'&&ent){ const kk=m.k; if(ent.pts>0&&ent.attr&&typeof ent.attr[kk]==='number'&&ent.attr[kk]<7){ ent.attr[kk]++; ent.pts--; } }
